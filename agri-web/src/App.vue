<template>
  <div class="app-container">
    <aside class="sidebar">
      ...
      <div class="sidebar-footer"><p>专家系统 v8.1 三分屏版</p></div>
    </aside>
              ...
              <div class="content-block advice-block card-inner hover-glow">
                ...
                <div class="advice-header-zone">
                  ...
                  <div class="advice-title-text">
                    <h4>🧠 专家多维诊断报告</h4>
                    <p>基于核心算法与 {{ selectedProvince }} 气候特征动态生成，请结合实际农情参考。</p>
                  </div>
                </div>
...
const generateExpertAdvice = (val, prov) => {
  ...
  if (val > 600) {
    ...
    list.push({
      ...
      icon: '🏆',
      title: '评级：S级丰收',
      content: `生长指标完美，预计大幅增产。请提前规划秋收物流与仓储。`
    })
  } else if (val > 450) {
    ...
    list.push({
      ...
      icon: '📊',
      title: '评级：A级平稳',
      content: `各项指标处于合理区间。当前无需人为干预，保持常态化田间巡视。`
    })
  } else {
    ...
    list.push({
      ...
      icon: '⚠️',
      title: '评级：C级高危',
      content: `系统监测到潜在的减产风险！请立即启动应急预案排查胁迫因子。`
    })
  }
  ...
  const interval = setInterval(() => {
    if (currentScore < baseScore) {
      currentScore += 2;
      healthScore.value = currentScore;
    } else {
      healthScore.value = baseScore;
  ...
  if (['新疆', '甘肃', '宁夏', '内蒙古'].includes(prov)) {
    list.push({
      ...
      icon: '💧',
      title: '水资源调度',
      content: '西北旱区水分胁迫极高，建议立即对接智能灌溉系统，实施定点滴灌。'
    })
    ...
    list.push({
      ...
      icon: '❄️',
      title: '冷害预警防控',
      content: '东北气象模型显示存在早霜风险，建议本周内完成抗逆诱抗剂喷施。'
    })
    ...
    list.push({
      ...
      icon: '🌧️',
      title: '农田排涝指令',
      content: '南方降水带偏多，建议启动排水泵站预案，确保沟渠畅通防渍水。'
    })
    ...
    list.push({
      ...
      icon: '🚜',
      title: '热害防治方案',
      content: '黄淮平原存在干热风隐患，建议利用无人机开展“一喷三防”作业。'
    })
  } else {
    list.push({
      type: 'normal',
      icon: '🛡️',
      title: '绿色植保防御',
      content: '根据病虫害扩散模型，建议在迎风坡面增设太阳能杀虫灯。'
    })
  }
  ...
  list.push({
    ...
    icon: '🧪',
    title: '测土配方施肥',
    content: '云端土壤数据显示营养失衡，建议追肥时依据配方表优化氮磷钾比例。'
  })
  list.push({
    type: 'normal',
    icon: '📈',
    title: '农业保险建议',
    content: '鉴于近期极端天气频发，建议为核心产区补充购买农业气象指数保险。'
  })
...
const initGaugeChart = (id) => {
  ...
  chart.setOption({
    series: [{
      ...
      detail: {formatter: '{value} kg', fontSize: 22, fontWeight: 'bolder', color: 'inherit', valueAnimation: true},
  ...
  chart.setOption({
    ...
    series: [{
      ...
      areaStyle: {
        ...
        }, {offset: 1, color: 'rgba(46, 213, 115, 0.0)'}])
      },
...
const initRankChart = () => {
  ...
  const sorted = categories.map((c, i) => ({
    name: c,
  ...
  myChart.setOption({
    ...
    series: [{
      ...
      barWidth: '40%',
      showBackground: true,
      backgroundStyle: {color: 'rgba(200, 200, 200, 0.1)', borderRadius: [10, 10, 0, 0]},
      itemStyle: {
        borderRadius: [10, 10, 0, 0],
        color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{offset: 0, color: '#4facfe'}, {
          offset: 1,
          color: '#00f2fe'
        }]),
...
const initMapChart = () => {
  ...
  chart.setOption({
    ...
    tooltip: {
      ...
      textStyle: {color: '#333'},
      formatter: (params) => provinceList.value.includes(params.data?.name) ? `<div style="font-weight:bold;color:#1890ff">${params.name}</div><div>点击启动 AI 推演</div>` : `<div style="color:#999">${params.name}</div>`
    },
    geo: {
      map: 'china',
      roam: true,
      zoom: 1.2,
      top: 'center',
      label: {show: true, color: '#666', fontSize: 10, fontWeight: 'bold'},
      itemStyle: {
        areaColor: '#f1f5f9',
        borderColor: '#cbd5e1',
        borderWidth: 1,
      ...
      emphasis: {
        ...
        itemStyle: {
          areaColor: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{
            offset: 0,
            color: '#2ed573'
          }, {offset: 1, color: '#1e90ff'}]), borderWidth: 0, shadowBlur: 20, shadowColor: 'rgba(46, 213, 115, 0.8)'
        }
      },
      select: {
        label: {color: '#fff'},
        itemStyle: {areaColor: '#ffa502', shadowBlur: 15, shadowColor: 'rgba(255, 165, 2, 0.6)'}
      }
    },
  ...
  chart.on('click', params => {
    const found = provinceList.value.find(dbName => normalizeName(dbName) === normalizeName(params.name));
    if (found) {
...
watch(currentView, (v) => {
  nextTick(() => {
    ...
    if (v === 'dashboard') setTimeout(initRankChart, 500)
  })
})
...
.hover-glow:hover {
  box-shadow: 0 8px 25px rgba(46, 213, 115, 0.15) !important;
...
.data-item strong {
  font-size: 26px;
  color: #2ed573;
  text-shadow: 0 2px 5px rgba(46, 213, 115, 0.2);
}
...
.modal-body {
  flex: 1;
  overflow-y: hidden; /* 防止外层滚动，只让内部块级滚动 */
  padding: 30px;
}
...
.layout-left-col .content-block {
  flex: 1; /* 左侧两块平分高度 */
}

.advice-block {
  flex: 1;
  overflow: hidden; /* 右侧充满，内部可滚动 */
  padding: 30px;
}
...
.health-badge-wrapper.success {
  background: linear-gradient(135deg, #a8ff78 0%, #78ffd6 100%);
  box-shadow: 0 10px 25px rgba(120, 255, 214, 0.4);
}

.health-badge-wrapper.info {
  background: linear-gradient(135deg, #89f7fe 0%, #66a6ff 100%);
  box-shadow: 0 10px 25px rgba(102, 166, 255, 0.4);
}

.health-badge-wrapper.warning {
...
.spacious-list::-webkit-scrollbar {
  width: 8px;
}

.spacious-list::-webkit-scrollbar-thumb {
  background: #cbd5e1;
  border-radius: 10px;
}
...
.advice-list-item.success {
  background: linear-gradient(90deg, rgba(46, 213, 115, 0.08), rgba(46, 213, 115, 0.01));
  border-color: rgba(46, 213, 115, 0.2);
}

.advice-list-item.success .item-icon-wrapper {
  background: rgba(46, 213, 115, 0.15);
  color: #27ae60;
}

.advice-list-item.info {
  background: linear-gradient(90deg, rgba(24, 144, 255, 0.08), rgba(24, 144, 255, 0.01));
  border-color: rgba(24, 144, 255, 0.2);
}

.advice-list-item.info .item-icon-wrapper {
  background: rgba(24, 144, 255, 0.15);
  color: #0984e3;
}

.advice-list-item.warning {
  background: linear-gradient(90deg, rgba(255, 165, 2, 0.08), rgba(255, 165, 2, 0.01));
  border-color: rgba(255, 165, 2, 0.2);
}

.advice-list-item.warning .item-icon-wrapper {
  background: rgba(255, 165, 2, 0.15);
  color: #e17055;
}

.advice-list-item.normal {
  background: linear-gradient(90deg, rgba(112, 161, 255, 0.08), rgba(112, 161, 255, 0.01));
  border-color: rgba(112, 161, 255, 0.2);
}

.advice-list-item.normal .item-icon-wrapper {
...
.fade-up {
  animation: fadeUp 0.4s cubic-bezier(0.165, 0.84, 0.44, 1);
}
